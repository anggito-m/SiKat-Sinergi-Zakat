-- Migration to handle Mayar webhook operations securely
-- This function runs as SECURITY DEFINER, meaning it bypasses RLS
-- and runs with the privileges of the function creator (admin).
    ...
    IF p_metadata IS NOT NULL THEN
        ...
        IF v_jenis IS NOT NULL AND v_muzakki_id IS NOT NULL THEN
            IF v_jenis = 'fitrah' THEN
                ...
                INSERT INTO zakat_fitrah (muzakki_id, jumlah_anggota, total_zakat, metode_pembayaran, status, mayar_payment_id)
                VALUES (v_muzakki_id, v_anggota, p_amount, 'mayar', 'selesai', p_payment_id);
                RETURN '{"success": true, "message": "New Zakat Fitrah inserted from AI chat"}'::jsonb;
                
            ELSIF v_jenis = 'mal' THEN
                INSERT INTO zakat_mal (muzakki_id, total_zakat, metode_pembayaran, status, mayar_payment_id)
